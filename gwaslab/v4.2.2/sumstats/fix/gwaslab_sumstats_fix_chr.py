...
def main():
    parser = argparse.ArgumentParser(
        description="Standardize chromosome notation and handle special chromosomes (X, Y, MT) in GWASLab.",
    ...
    parser.add_argument("--add_prefix", type=str, default="", help="Prefix to prepend to chromosome labels (e.g. 'chr').")
    parser.add_argument("--remove", action="store_true", default=False, help="Remove records with unrecognized chromosome labels.")

    args = parser.parse_args()

    try:
        mysumstats = load_sumstats(args)
        mysumstats.fix_chr(add_prefix=args.add_prefix, remove=args.remove)
        save_sumstats(mysumstats, args.out, out_fmt=args.out_fmt)
        print(f"Successfully fixed chromosomes and saved to {args.out}")
    except Exception as e:
        print(f"Error in fix_chr: {e}", file=sys.stderr)
        sys.exit(1)
