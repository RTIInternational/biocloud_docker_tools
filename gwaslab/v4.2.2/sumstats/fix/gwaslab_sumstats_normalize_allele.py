#!/usr/bin/env python3
"""
GWASLab Sumstats Fix: normalize_allele
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Normalize indel alleles via left-alignment and parsimony trimming in GWASLab.",
    ...
    parser.add_argument("--threads", type=int, default=1, help="Number of threads for parallel normalization.")

    args = parser.parse_args()

    try:
        mysumstats = load_sumstats(args)
        mysumstats.normalize_allele(threads=args.threads)
        save_sumstats(mysumstats, args.out, out_fmt=args.out_fmt)
        print(f"Successfully normalized alleles and saved to {args.out}")
    except Exception as e:
        print(f"Error in normalize_allele: {e}", file=sys.stderr)
        sys.exit(1)
