#!/usr/bin/env python3
"""
GWASLab Sumstats Fix: check_data_consistency
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Check consistency between related statistical values in GWASLab.",
    ...
    try:
        ...
        mysumstats.check_data_consistency(rtol=args.rtol, atol=args.atol)
        save_sumstats(mysumstats, args.out, out_fmt=args.out_fmt)
        print(f"Successfully checked data consistency and saved to {args.out}")
    except Exception as e:
        print(f"Error in check_data_consistency: {e}", file=sys.stderr)
        sys.exit(1)
