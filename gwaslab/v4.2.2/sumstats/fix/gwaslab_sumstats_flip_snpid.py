#!/usr/bin/env python3
"""
GWASLab Sumstats Fix: flip_snpid & strip_snpid
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Flip or strip SNPID strings in GWASLab.",
    ...
    parser.add_argument("--action", type=str, default="flip", choices=["flip", "strip"], help="Action to perform on SNPID ('flip' to swap EA/NEA, 'strip' to remove non-standard prefixes/suffixes).")
    ...
    try:
        ...
        if args.action == "flip":
            mysumstats.flip_snpid()
        elif args.action == "strip":
            mysumstats.strip_snpid()
        save_sumstats(mysumstats, args.out, out_fmt=args.out_fmt)
        print(f"Successfully performed {args.action}_snpid and saved to {args.out}")
    except Exception as e:
        print(f"Error in {args.action}_snpid: {e}", file=sys.stderr)
        sys.exit(1)
