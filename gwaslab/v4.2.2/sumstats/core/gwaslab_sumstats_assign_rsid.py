#!/usr/bin/env python3
"""
GWASLab Sumstats Core: assign_rsid
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Assign rsIDs to variants using a reference VCF or TSV file in GWASLab.",
    ...
    parser.add_argument("--ref_rsid_tsv", type=str, default=None, help="Path to reference rsID TSV file.")
    parser.add_argument("--ref_rsid_vcf", type=str, default=None, help="Path to reference rsID VCF/BCF file.")
    parser.add_argument("--overwrite", type=str, default="empty", choices=["all", "invalid", "empty"], help="Overwrite mode for rsID assignment.")
    parser.add_argument("--threads", type=int, default=1, help="Number of threads for parallel matching.")

    args = parser.parse_args()

    if not args.ref_rsid_tsv and not args.ref_rsid_vcf:
        print("Error: Either --ref_rsid_tsv or --ref_rsid_vcf must be provided.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print(f"Successfully assigned rsIDs and saved to {args.out}")
    except Exception as e:
        print(f"Error in assign_rsid: {e}", file=sys.stderr)
        sys.exit(1)
