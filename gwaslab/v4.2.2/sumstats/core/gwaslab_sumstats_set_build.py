#!/usr/bin/env python3
"""
GWASLab Sumstats Core: set_build
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Set genome build in STATUS column and metadata using GWASLab.",
    ...
    parser.add_argument("--target_build", required=True, type=str, help="Genome build version to set ('19', '38', '99').")

    args = parser.parse_args()

    try:
        mysumstats = load_sumstats(args)
        mysumstats.set_build(args.target_build)
        save_sumstats(mysumstats, args.out, out_fmt=args.out_fmt)
        print(f"Successfully set build to {args.target_build} and saved to {args.out}")
    except Exception as e:
        print(f"Error in set_build: {e}", file=sys.stderr)
        sys.exit(1)
