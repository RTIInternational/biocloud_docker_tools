#!/usr/bin/env python3
"""
GWASLab Sumstats Core: check_ref
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Check allele alignment of non-effect allele (NEA) with reference FASTA in GWASLab.",
    ...
    parser.add_argument("--ref_seq", required=True, type=str, help="Path to reference genome FASTA file.")
    parser.add_argument("--remove", action="store_true", default=False, help="Remove variants not found on the reference genome.")

    args = parser.parse_args()

    try:
        mysumstats = load_sumstats(args)
        mysumstats.check_ref(ref_seq=args.ref_seq, remove=args.remove)
        save_sumstats(mysumstats, args.out, out_fmt=args.out_fmt)
        print(f"Successfully checked reference and saved to {args.out}")
    except Exception as e:
        print(f"Error in check_ref: {e}", file=sys.stderr)
        sys.exit(1)
