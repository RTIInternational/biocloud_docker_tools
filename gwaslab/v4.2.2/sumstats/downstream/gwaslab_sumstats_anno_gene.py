#!/usr/bin/env python3
"""
GWASLab Sumstats Downstream: anno_gene
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Annotate summary statistics variants with nearest gene names in GWASLab.",
    ...
    try:
        ...
        mysumstats.anno_gene()
        save_sumstats(mysumstats, args.out, out_fmt=args.out_fmt)
        print(f"Successfully annotated gene names and saved to {args.out}")
    except Exception as e:
        print(f"Error in anno_gene: {e}", file=sys.stderr)
        sys.exit(1)
