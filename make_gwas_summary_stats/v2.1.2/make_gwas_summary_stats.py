...
def readInfoChunk(infoFileHandle, chunkSize, infoColXref, infoFormat):
    ...
    infoChunk.CHR = infoChunk.CHR.astype(str).replace({'chr':''}, regex=True)
    infoChunk.CHR = infoChunk.CHR.astype(str).replace({'^0':''}, regex=True)
    ...
    if infoFormat == "vcf" and 'INFO' in infoChunk.columns:
        
        if infoChunk['INFO'].str.contains('R2=').any():
            infoChunk['IMP_QUAL'] = infoChunk['INFO'].str.extract(r'R2=(.*?)(;|$)', expand=False)[0].astype(float)
        else:
            infoChunk['IMP_QUAL'] = np.nan
        if infoChunk['INFO'].str.contains('IMPUTED').any():
            infoChunk['SOURCE'] = 'IMP'
        elif infoChunk['INFO'].str.contains('TYPED').any():
            infoChunk['SOURCE'] = 'GEN'
        else:
            infoChunk['SOURCE'] = 'NA'
    elif 'SOURCE' in infoChunk.columns:
        sourceXref = {
            'Imputed': 'IMP',
            'Genotyped': 'GEN',
            'Typed_Only': 'GEN'
    ...
    print(infoChunk.head())
    return infoChunk

def get_first_matching_line_number(filepath, pattern):
    """
    Finds the line number of the first occurrence of a pattern in a file.
    ...
    Returns:
        int or None: The 1-based line number of the first matching line,
    ...
    if filepath.endswith('.gz'):
        ...
        open_func = open
        read_mode = 'r'
    with open_func(filepath, read_mode) as f:
        for line_num, line in enumerate(f, 1):  # Start enumeration from 1 for 1-based line numbers
...
if args.file_in_info_format == "info":
...
elif args.file_in_info_format == "vcf":
    # Define Xref for renaming info columns
    infoColXref = {
        "ID": 'VARIANT_ID',
        "#CHROM": 'CHR',
    ...
    info = pd.read_csv(
        ...
        usecols = infoColXref.keys(),
        iterator = True,
        skiprows = get_first_matching_line_number(args.file_in_info, r'^#CHROM') - 1
    )
