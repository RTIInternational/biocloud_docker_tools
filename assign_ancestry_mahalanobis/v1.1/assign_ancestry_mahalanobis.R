...
option_list = list(
    ...
    make_option(
        ...
        default='2,3,4',
        type='character',
        help="StdDev thresholds for filtering ancestries, separated by commas"
    ),
    make_option(
        c('--scale-to-ref'),
        action='store_true',
...
sd_cutoffs = as.numeric(unlist(strsplit(get_arg(args, 'std-dev-cutoffs'), split = ",")))
...
# Create keep lists for each ancestry, summary, and plots for different # of standard deviations
std_dev_cols = c()
for (sd in sd_cutoffs) {
    new_col = paste0('ANCESTRY_', sd, '_StdDev')
    std_dev_cols = c(std_dev_cols, new_col)
    dataset_samples[dataset_samples$SCALED_MAHAL <= sd, new_col] = dataset_samples[dataset_samples$SCALED_MAHAL <= sd, 'ANCESTRY']
    dataset_samples[dataset_samples$SCALED_MAHAL > sd, new_col] = 'Unassigned'
    # Write keep lists
    for (ancestry in ancestries) {
    ...
    plot_data = plot_data[!is.na(plot_data$SCALED_MAHAL),]
...
# Write table of dataset ancestry assignments with stddev filtering
write.table(
    dataset_samples[, c('FID', 'IID', 'POP', ancestries, 'SCALED_MAHAL', 'ANCESTRY', std_dev_cols)],
    file=paste0(out_dir, dataset, '_ancestry_assignments.tsv'),
    quote=FALSE,
    sep="\t",
    row.names=F
)
