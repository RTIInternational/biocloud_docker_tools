...
if (nchar(working_dir) > 0 && substr(working_dir, nchar(working_dir), nchar(working_dir)) != '/') {
...
chunk_count = get_arg(args, "nchunks")
...
## Split imputed data into chunks
num_snps_vector =seq(1:length(all_snp_ids[[1]]))
...
for (index in 1:chunk_count) {
    ## Get chunk to run
    chunk_to_run = list_chunks[[index]]
    start = chunk_to_run[1]; end = chunk_to_run[length(chunk_to_run)]
    index_snps =start:end

    ## Subset bigsnpr file to chunk. Note this creates a _sub file so make sure to delete afterwards
    print("Creating temp rds/bk files")
    chunk_rds =bigsnpr::snp_subset(ukb.bigSNP, ind.col = index_snps, backingfile = sprintf("%s/sub%s", working_dir, index)) #  Subset snps to chunk. Keep all people
    chunk_subset.bigSNP =bigsnpr::snp_attach(chunk_rds)

    ## Get chunk genotype matrix
    print("Reading genotype matrix")
    snp_ids =chunk_subset.bigSNP$map$rsid
    genotypes =as.data.frame(chunk_subset.bigSNP$genotypes[])
    colnames(genotypes) = snp_ids
    genotypes$IID =samp_file[-1,]$ID_1 #1st row has 0, 0, 0, D

    ## Create df with phenotype and chunk of genotypes to run
    merged =merge(with_geno_pheno, genotypes, by = 'IID')

    print("Performing association testing")
    ...
    final_output = merge(as.data.frame(ukb.bigSNP$map[,c("chromosome", "rsid", "physical.pos", "allele1", "allele2", "freq", "info")]), all_output)
    ...
    system(sprintf("rm %ssub%s*", working_dir, index))
